...
/* const ResetPasswordForm: React.FC<NewPasswordProps> = ({
  ...
  const [searchParams] = useSearchParams();
  const resetCode = searchParams.get("code");
  const resetCodeExpiration = Number(searchParams.get("exp"));
  const currentDate = Date.now();

  const usedCodes = JSON.parse(localStorage.getItem("usedResetCodes") || "[]");

  const isCodeValid =
  resetCode &&
  resetCodeExpiration &&
  resetCodeExpiration > currentDate &&
  !usedCodes.includes(resetCode);
  ...
  const {
    ...
    onSuccess: () =>{
      ...
      localStorage.setItem("usedResetCodes", JSON.stringify([...usedCodes, resetCode]));
  ...
  return(
    <section className="flex items-center justify-center bg-gray-50">
      <section className={isError ? "" : "w-full max-w-md space-y-8 bg-gray-100 p-6 rounded-lg shadow-lg"}>
        {mutationError || !isCodeValid ? (
          <section role="alert" className="max-w-fit rounded-xl alert alert-error text-center p-4 text-red-600 bg-red-100">
            <RxCrossCircled className="inline-block mr-2"/>
            {mutationError instanceof Error ? mutationError.message : "Unknown error"}
          </section>
        ) : (
          <>
